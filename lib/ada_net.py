...
def matching_max_mean(img_regions, cap_words, cap_len, i2t=False, scan=False, bi_norm=False):
    ...
    for i in range(n_caption):
        if img_regions_context is not None:
...
# img_mask (B_v, L_v)
def mask_xattn_one_text(img_embs, cap_i_expand, img_mask=None, i2t=True, scan=True, fusion_scale=5.0):
    """
    自适应双向融合版本：
    - 保持原始 max 匹配
    - 仅把最终 row / column 融合从固定相加改成样本级自适应加权
    """
    # (B_v, L_t, L_v)
    ...
    if i2t:
        ...
        # adaptive fusion:
        # row_sim_mean 更大时更偏向 row，反之更偏向 column
        alpha = torch.sigmoid(fusion_scale * (row_sim_mean - column_sim_mean))
        sim_one_text = alpha * row_sim_mean + (1 - alpha) * column_sim_mean
    else:
