...
def get_argument_parser():
    ...
    parser.add_argument('--embedding_warmup_epochs', type=int, default=0,
                        help='The number of epochs for warming up the embedding layer')

    parser.add_argument('--f30k_img_path', type=str, default='/root/autodl-tmp/seps-main/data/flickr30k-images',
                        help='the path of f30k images')
    parser.add_argument('--coco_img_path', type=str, default='/root/autodl-tmp/seps-main/data/coco-images',
                        help='the path of coco images')
    ...
    # optimizer
    parser.add_argument("--lr_schedules", default=[12, 18, 24], type=int, nargs="+",
                        help='epoch schedules for lr decay')
    parser.add_argument("--decay_rate", default=0.5, type=float, help='lr decay_rate for optimizer')
    ...
    parser.add_argument('--attention_weight', type=int, default=0.8,
                        help='the weight of attention_map for mask prediction')
    parser.add_argument('--ratio_weight', type=float, default=2.0, help='if use detach for kt loss')

    # conservative learnable score
    parser.add_argument('--score_pred_hidden', type=int, default=128, help='hidden dim of dense score delta predictor')
    parser.add_argument('--score_delta_scale', type=float, default=0.1,
                        help='delta scale for conservative learnable score')
